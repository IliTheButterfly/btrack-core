...
public:
    ...
    {
        _MEM_DEBUG(std::cout << "Copied " << typeid(T).name() << " from " << __PRETTY_FUNCTION__ << std::endl;)
        if (_ItemValueAccessor<U>::getPtr(val))
            mPtr = std::make_shared<T>(*(_ItemValueAccessor<U>::getPtr(val)));
        else
            mPtr.reset();
        return *this;
    }
    ...
    {
        _MEM_DEBUG(std::cout << "Copied " << typeid(T).name() << " from " << __PRETTY_FUNCTION__ << std::endl;)
        if (_ItemViewAccessor<U>::getPtr(view))
            mPtr = std::make_shared<T>(*(_ItemViewAccessor<U>::getPtr(view)));
        else
            mPtr.reset();
        return *this;
    }
...
public:
    ...
    ItemValue<T> &copyFrom(const ItemValue<U> &val) noexcept
    {
        _MEM_DEBUG(std::cout << "Copied " << typeid(T).name() << " from " << __PRETTY_FUNCTION__ << std::endl;)
        if (_ItemValueAccessor<U>::getPtr(val))
            mPtr = std::make_unique<T>(*(_ItemValueAccessor<U>::getPtr(val)));
        else
            mPtr.reset();
        return *this;
    }
    ...
    ItemValue<T> &copyFrom(const ItemView<U> &view) noexcept
    {
        _MEM_DEBUG(std::cout << "Copied " << typeid(T).name() << " from " << __PRETTY_FUNCTION__ << std::endl;)
        if (_ItemViewAccessor<U>::getPtr(view))
            mPtr = std::make_unique<T>(*(_ItemViewAccessor<U>::getPtr(view)));
        else
            mPtr.reset();
        return *this;
    }
...
template <>                                                     \
struct DefaultChannelTypeInfo<T>                                \
{                                                               \
    using type = T;                                             \
    using item = T;                                             \
...
public:
    ...
    {
        boost::lock_guard<mutex> lock(mMTX);
        auto i = std::find_if(mSenders.begin(), mSenders.end(), [&](SndPtr p)
                                { return !p.expired() && p.lock() == sender; });
        if (i == mSenders.end())
        {
    ...
    void send(typename I::readonlyIn data) override
    {
        ...
        {
            if (!sender.expired())
                sender.lock()->send(data);
            else
                remove = i;
            ++i;
        }
        if (remove != -1)
            mSenders.erase(mSenders.begin() + remove);
    }
    ...
    void sendAndTake(typename I::writableIn data) override
    {
        ...
        {
            if (!mSenders[0].expired())
                mSenders[0].lock()->sendAndTake(std::move(data));
            else
                mSenders.clear();
        }
        else
